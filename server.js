...
app.post('/api/extract', async (req, res) => {
    try {
        ...
        // 1. Filtro de Tamaño Físico (Rechaza si pesa menos de 40 KB)
        ...
        if (sizeInKB < 40) {
            ...
            return res.json({ 
                ...
                mensajeRechazo: "La imagen es muy pequeña. Por favor, toma la foto más de cerca." 
            });
        }

        // 2. Ejecutar Google Cloud Vision
        const [result] = await client.textDetection(imageBuffer);
        
        // 3. Filtro de Claridad (Confidence Score de Google)
        let confidence = 0;
        if (result.fullTextAnnotation && result.fullTextAnnotation.pages.length > 0) {
            // Google califica la claridad de lectura de 0.0 a 1.0
            confidence = result.fullTextAnnotation.pages[0].confidence || 0; 
        }

        // Si Google está menos del 80% (0.80) seguro, significa que está borrosa o movida
        if (confidence < 0.80) {
        ...
        // Si pasó las pruebas de calidad, procedemos con la Fase 2 normal:
        ...
        if (idxApellidos > 0) { apellidosExtraidos = lineas[idxApellidos - 1]; }

        let nombresExtraidos = "No detectados";
        const idxNombres = lineas.findIndex(l => l.includes('NOMBRE'));
        if (idxNombres > 0) { nombresExtraidos = lineas[idxNombres - 1]; }

        // Enviamos el éxito a Genesys
        res.json({
